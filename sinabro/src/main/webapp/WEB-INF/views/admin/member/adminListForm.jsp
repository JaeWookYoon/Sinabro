...
<script type="text/javascript">
	function search() {
		...
		var url = 'adminList.mustang?pageNum = 1&search_type='
...
<body>
	<center>
				...
				<c:if test="${count !=0 }">
					<c:forEach var="vo" items="${adminList }">
						<tr>
							...
							<td align="center"><b>
							<a href="adminInformation.mustang?id=${vo.id}&pageNum=${bp.currentPage} ">${vo.id }</a></b>
							</td>
		...
		<c:if test="${bp.isPre() }">
			<a
				href="adminList.mustang?pageNum=${bp.getPage_Start()-bp.pageBlock }">[Previous]</a>
		</c:if>
		<c:forEach var="counter" begin="${bp.getPage_Start() }"
			end="${endPage}">
			<c:if test="${search_text ne ''}">
				<a
					href="javascript:window.location='adminList.mustang?pageNum=${counter }&search_type=${search_type}&search_text=${search_text}'">
					[${counter }]</a>
			</c:if>
			<c:if test="${search_text eq ''}">
				<a
					href="javascript:window.location='adminList.mustang?pageNum=${counter }'">
					[${counter }]</a>
			</c:if>
		</c:forEach>
		<c:if test="${bp.isNext() }">
			<a
				href="adminList.mustang?pageNum=${bp.getPage_Start()+bp.pageBlock }">[Next]</a>
		</c:if>
		<p>
		<p>
		<div id="search">
			<form method="get" name="listForm" action="adminList.mustang">
